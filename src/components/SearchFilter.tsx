...
export default function SearchFilter({
  ...
  unitFilter, onUnitChange, units, doseFilter, onDoseChange, doses,
}: SearchFilterProps) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-4 flex flex-col sm:flex-row gap-4 transition-colors duration-300">
      ...
      <div className="relative">
        ...
        <select value={statusFilter} onChange={(e) => onStatusChange(e.target.value)}
          className={`pr-10 pl-8 py-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all min-w-[180px] cursor-pointer ${
            statusColor ? `${statusColor.bgColor} ${statusColor.color} border-transparent font-medium` : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-white'
          }`}>
          {allStatuses.map((s) => (
      ...
      {doses && onDoseChange && (
        <div className="relative">
          ...
          <select value={doseFilter ?? ''} onChange={(e) => onDoseChange(e.target.value)}
      ...
      {units && onUnitChange && (
        <div className="relative">
          <select value={unitFilter ?? ''} onChange={(e) => onUnitChange(e.target.value)}
