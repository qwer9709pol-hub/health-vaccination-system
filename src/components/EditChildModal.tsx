...
export default function EditChildModal({ child, isOpen, onClose, onSave }: EditChildModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (status === 'تم التطعيم فى وحدة بتاريخ') {
      if (!vaccinationPlace.trim()) {
        setError('من فضلك أدخل مكان التطعيم');
        setLoading(false);
        return;
      }
      if (!vaccinationDate) {
        setError('من فضلك أدخل تاريخ التطعيم');
        setLoading(false);
        return;
      }
    }
    ...
    if (status === 'مسافر' || status === 'مسافر موثق') {
      if (!travelCountry.trim()) {
        setError('من فضلك أدخل دولة السفر');
        setLoading(false);
        return;
      }
      if (!travelDate) {
        setError('من فضلك أدخل تاريخ السفر');
        setLoading(false);
        return;
      }
    }

    if (status === 'مريض' && !diseaseName.trim()) {
      setError('من فضلك أدخل اسم المرض');
      setLoading(false);
      return;
    }

    try {
      const updates: Partial<DelayedChild> = {
        status,
      ...
      if (status === 'مريض') {
        (updates as any).disease_name = diseaseName;
      }
      if (status === 'تم التحويل الى اقرب وحدة') {
        (updates as any).transfer_destination = transferDestination;
      }
      if (status === 'رفض') {
        (updates as any).refusal_reason = refusalReason;
      }
      if (status === 'متوفى') {
        (updates as any).death_date = deathDate;
      }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
      <div
        className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transition-colors duration-300"
        dir="rtl"
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-2">
            <p className="text-sm text-gray-600 dark:text-gray-300">
              <span className="font-medium text-gray-700 dark:text-gray-200">اسم الطفل:</span> {child.child_name}
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              <span className="font-medium text-gray-700 dark:text-gray-200">رقم القيد:</span> {child.registration_number || '-'}
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              <span className="font-medium text-gray-700 dark:text-gray-200">رقم الهاتف:</span> {child.phone_number || '-'}
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              <span className="font-medium text-gray-700 dark:text-gray-200">التطعيم المتخلف:</span> {child.dose || 'غير محدد'}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={status}
                onChange={(e) => handleStatusChange(e.target.value as ChildStatus)}
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded-lg p-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setVaccinationPlace(e.target.value)}
                  className="w-full px-4 py-3 border border-emerald-300 dark:border-emerald-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 dark:text-white"
          ...
          {status === 'مريض' && (
            <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-red-300 dark:border-red-700 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 dark:text-white"
          ...
          {status === 'تم التحويل الى اقرب وحدة' && (
            <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-blue-300 dark:border-blue-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 dark:text-white"
                placeholder="اسم الوحدة"
              />
            </div>
          )}
          ...
          {status === 'رفض' && (
            <div className="bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 border border-orange-300 dark:border-orange-700 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 dark:text-white"
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none resize-none bg-white dark:bg-gray-700 dark:text-white"
