...
export default function AnalyticsPage() {
  ...
  const doseDistribution = useMemo(() => {
    ...
    children.forEach((c) => { if (c.dose) doseMap.set(c.dose, (doseMap.get(c.dose) || 0) + 1); });
    return Array.from(doseMap.entries()).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
  }, [children]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-6">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-emerald-600" />أداء الوحدات (أعلى 10)</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topUnits.map((u) => ({ name: u.unit_name.length > 12 ? u.unit_name.substring(0, 12) + '...' : u.unit_name, 'تم التطعيم': u.vaccinated, 'المتبقي': u.remaining }))} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                <XAxis type="number" stroke="#9ca3af" />
                <YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 10, fill: '#9ca3af' }} />
                <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '8px' }} />
                <Legend />
                <Bar dataKey="تم التطعيم" stackId="a" fill="#059669" />
                <Bar dataKey="المتبقي" stackId="a" fill="#f59e0b" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-6">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-blue-600" />توزيع الجرعات</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={doseDistribution}>
                <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                <XAxis dataKey="name" stroke="#9ca3af" tick={{ fontSize: 10, fill: '#9ca3af' }} angle={-20} textAnchor="end" height={60} />
                <YAxis stroke="#9ca3af" />
                <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '8px' }} />
                <Bar dataKey="value" fill="#3b82f6" name="عدد الأطفال" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-6">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Award className="w-5 h-5 text-amber-600" />ترتيب الوحدات حسب الإنجاز</h3>
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {unitStats.sort((a, b) => b.completion - a.completion).map((stat, index) => (
              <div key={stat.unit_id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${index === 0 ? 'bg-amber-100 text-amber-700' : index === 1 ? 'bg-gray-200 text-gray-700' : index === 2 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>{index + 1}</span>
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{stat.unit_name}</p>
                ...
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{stat.completion}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {lowUnits.length > 0 && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 dark:text-red-400 mt-1">{lowUnits.map((u) => u.unit_name).join('، ')} - نسبة إنجاز أقل من 50%</p>
          </div>
        </div>
      )}
    </div>
