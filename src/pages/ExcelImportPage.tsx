...
export default function ExcelImportPage() {
  ...
  const handleImport = async () => {
    if (!fileInputRef.current?.files?.[0]) { setError('من فضلك اختر ملف أولاً'); return; }
    ...
    try {
      ...
      if (rows.length === 0) { setError('الملف فارغ'); setImporting(false); return; }
      ...
      for (const row of rows) {
        try {
          ...
          if (!unitId) { errors++; errorMessages.push(`الوحدة "${unitName}" غير موجودة`); continue; }
          ...
          const { error: insertError } = await supabase.from('delayed_children').insert(childData);
          if (insertError) { errors++; errorMessages.push(`خطأ في صف "${childData.child_name}": ${insertError.message}`); }
          else success++;
        } catch (err: any) {
          errors++; errorMessages.push(`خطأ: ${err.message}`);
        }
      }
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-6">
        ...
        <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-3 mx-auto">
            <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-900/30 rounded-full flex items-center justify-center">
              <Upload className="w-8 h-8 text-emerald-600" />
            </div>
            <span className="text-gray-700 dark:text-gray-300 font-medium">{fileName || 'اختر ملف Excel'}</span>
          </button>
        </div>
        {error && (
        ...
        {result && (
          <div className="mt-4 space-y-3">
            <div className="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-lg">
              <CheckCircle className="w-5 h-5" /><span>تم استيراد {result.success} طفل بنجاح</span>
            </div>
            {result.errors > 0 && (
              <div className="p-3 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg">
                <div className="flex items-center gap-2 mb-2"><X className="w-5 h-5" /><span>{result.errors} خطأ</span></div>
                <ul className="text-sm space-y-1 mr-7">{result.errorMessages.slice(0, 10).map((msg, i) => (<li key={i}>{msg}</li>))}</ul>
              </div>
            )}
          </div>
        )}
        <button onClick={handleImport} disabled={!fileName || importing}
          ...
          {importing ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><Upload className="w-5 h-5" /><span>استيراد</span></>}
        </button>
      </div>
    </div>
  );
