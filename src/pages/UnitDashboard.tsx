...
export default function UnitDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      const childrenData = await fetchChildren(unitId);
      setChildren(childrenData);
      setKPIs(calculateKPIs(childrenData));
      if (childrenData.length > 0 && childrenData[0].unit) setUnit(childrenData[0].unit);
  ...
  const filteredChildren = useMemo(() => {
    return children.filter((child) => {
      ...
      const matchesSearch = !q || child.child_name?.toLowerCase().includes(q) || child.mother_name?.toLowerCase().includes(q) || child.phone_number?.includes(q) || child.reporter_phone?.includes(q) || child.registration_number?.toString().includes(q) || child.address?.toLowerCase().includes(q) || child.dose?.toLowerCase().includes(q);
