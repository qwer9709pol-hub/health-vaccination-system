...
export default function AdminDashboard() {
  ...
  const handleExport = () => {
    const exportData = filteredChildren.map((child) => ({
      'الوحدة': child.unit?.unit_name || '', 'اسم الطفل': child.child_name, 'اسم الأم': child.mother_name || '',
      'رقم القيد': child.registration_number || '', 'تاريخ الميلاد': formatDate(child.birth_date),
      'العمر': child.age ? `${child.age} سنة` : '', 'العنوان': child.address || '',
    ...
    ws['!cols'] = Array.from({ length: 24 }, () => ({ wch: 20 }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 p-6">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-emerald-600" />إحصائيات الوحدات (أعلى 10)</h3>
          <div className="h-64">
      ...
      {unitStats.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 overflow-hidden">
          <div className="px-6 py-4 border-b dark:border-gray-700"><h3 className="font-semibold text-gray-900 dark:text-white">إحصائيات جميع الوحدات</h3></div>
      ...
      <div className="space-y-4">
        ...
        <SearchFilter
          ...
          unitFilter={unitFilter} onUnitChange={setUnitFilter} units={units.map((u) => u.unit_name)}
          doseFilter={doseFilter} onDoseChange={setDoseFilter} doses={availableDoses}
        />
        <ChildrenTable
          children={filteredChildren} loading={loading} showUnit
